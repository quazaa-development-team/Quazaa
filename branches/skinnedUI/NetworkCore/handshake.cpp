...
void CHandshake::OnTimer(quint32 tNow)
{
	...
	{
		systemLog.postLog(qApp->tr("Timed out handshaking with  %1").arg(m_pSocket->peerAddress().toString().toAscii().constData()), LogSeverity::Debug);
...
void CHandshake::OnRead()
{
	...
	{
		systemLog.postLog(qApp->tr("Incoming connection from %1 is Gnutella Neighbour connection").arg(m_pSocket->peerAddress().toString().toAscii().constData()), LogSeverity::Debug);
	...
	{
		systemLog.postLog(qApp->tr("Closing connection with %1 - unknown protocol").arg(m_pSocket->peerAddress().toString().toAscii().constData()), LogSeverity::Debug);
