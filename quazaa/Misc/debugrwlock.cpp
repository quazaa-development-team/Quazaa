...
{
	qDebug() << "RWLock ID #" << QString::number( nLockID ).toLocal8Bit().data() << ": read lock requested    - caller: " << ( caller ? caller : "" );
	QReadWriteLock::lockForRead();
	qDebug() << "RWLock ID #" << QString::number( nLockID ).toLocal8Bit().data() << ": read lock obtained     - caller: " << ( caller ? caller : "" );
}

void	CDebugRWLock::lockForWrite(const char *caller)
{
	qDebug() << "RWLock ID #" << QString::number( nLockID ).toLocal8Bit().data() << ": write lock requested   - caller: " << ( caller ? caller : "" );
	QReadWriteLock::lockForWrite();
	qDebug() << "RWLock ID #" << QString::number( nLockID ).toLocal8Bit().data() << ": write lock obtained    - caller: " << ( caller ? caller : "" );
}

bool	CDebugRWLock::tryLockForRead(const char *caller)
{
	qDebug() << "RWLock ID #" << QString::number( nLockID ).toLocal8Bit().data() << ": try read lock          - caller: " << ( caller ? caller : "" );
...
bool	CDebugRWLock::tryLockForRead(int timeout, const char *caller)
{
...
bool	CDebugRWLock::tryLockForWrite(int timeout, const char *caller)
{
	qDebug() << "RWLock ID #" << QString::number( nLockID ).toLocal8Bit().data() << ": try read lock - timeout: " << QString::number( timeout ).toLocal8Bit().data();
...
void	CDebugRWLock::unlock(const char *caller)
{
