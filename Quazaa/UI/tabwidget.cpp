...
QKeySequence CTabBar::navigationShortcut( Navigation navigation ) const
{
	...
	case NextUnread:
		return nextUnreadShortcut->key();
	case PreviousUnread:
		return prevUnreadShortcut->key();
	default:
		return QKeySequence();
	}
}
...
void CTabBar::setNavigationShortcut( Navigation navigation, const QKeySequence& shortcut )
{
	...
	{
	case Next:
		return nextShortcut->setKey( shortcut );
	case Previous:
		return prevShortcut->setKey( shortcut );
	case NextUnread:
		return nextUnreadShortcut->setKey( shortcut );
	case PreviousUnread:
		return prevUnreadShortcut->setKey( shortcut );
	}
}
...
void CTabWidget::alertTimeout()
{
	...
	foreach ( const int& nIndex, d.alertIndexes )
	{
		colorizeTab( nIndex );
	}
}
