...
{
	QString message = tr("%1 %2 - http://%3").arg(qApp->applicationName())
...
IrcCommand* Connection::createCtcpReply(IrcPrivateMessage* request) const
{
	...
	if (cmd == "VERSION") {
		const QString message = tr("%1 %2").arg(SingleApplication::applicationName())
										   .arg(SingleApplication::applicationVersion());
