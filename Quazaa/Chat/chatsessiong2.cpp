...
void CChatSessionG2::onConnectNode()
{
	...
	baHs += "Listen-IP: " + networkG2.getLocalAddress().toStringWithPort() + "\r\n\r\n";
...
void CChatSessionG2::send_ChatOK( bool bReply )
{
	...
	if ( !bReply )
	{
		...
		sHs += "Listen-IP: " + networkG2.getLocalAddress().toStringWithPort() + "\r\n";
	}
...
void CChatSessionG2::onUPROC( G2Packet* pPacket )
{
	...
	QString sXML =
		"<?xml version=\"1.0\"?><gprofile xmlns=\"http://www.shareaza.com/schemas/GProfile.xsd\"><gnutella guid=\"%1\"/><identity><handle primary=\"%2\"/></identity></gprofile>";
	sXML = sXML.arg( quazaaSettings.Profile.GUID.toString().toUpper().replace( "{", "" ).replace( "}",
																								  "" ) ).arg( quazaaSettings.Profile.GnutellaScreenName );
