...
struct IrcSettings {
	...
	bool operator==(const IrcSettings& other) const;
	bool operator!=(const IrcSettings& other) const;
...
Q_DECLARE_METATYPE(IrcSettings);

inline QDataStream& operator<<(QDataStream& out, const IrcSettings& settings)
{
...
inline QDataStream& operator>>(QDataStream& in, IrcSettings& settings)
{
	...
	IrcSettings defaults; // default values
	if (version < 125)
		settings.colors[IrcSettings::TimeStamp] = defaults.colors.value(IrcSettings::TimeStamp);
	if (version < 126)
		settings.layout = defaults.layout;
	if (version < 127) {
		settings.shortcuts[IrcSettings::NextUnreadUp] = defaults.shortcuts.value(IrcSettings::NextUnreadUp);
		settings.shortcuts[IrcSettings::NextUnreadDown] = defaults.shortcuts.value(IrcSettings::NextUnreadDown);
		settings.shortcuts[IrcSettings::NextUnreadLeft] = defaults.shortcuts.value(IrcSettings::NextUnreadLeft);
		settings.shortcuts[IrcSettings::NextUnreadRight] = defaults.shortcuts.value(IrcSettings::NextUnreadRight);
	}
	if (version < 128)
		settings.colors[IrcSettings::Link] = defaults.colors.value(IrcSettings::Link);
	if (version < 129)
