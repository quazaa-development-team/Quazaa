...
{
	ui.setupUi(this);
	setPixmap(QWizard::LogoPixmap, QPixmap(":/Resource/oxygen/64x64/actions/forward.png"));
...
QHash<int, QString> ShortcutsWizardPage::shortcuts() const
{
	...
	for (int i = IrcSettings::NavigateUp; i <= IrcSettings::NavigateRight; ++i) {
	...
	for (int i = IrcSettings::NextUnreadUp; i <= IrcSettings::NextUnreadRight; ++i) {
		QTreeWidgetItem* item = unread->child(i - IrcSettings::NextUnreadUp);
		shortcuts.insert(i, item->text(Shortcut));
	}
