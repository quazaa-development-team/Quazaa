...
void CHostCache::save()
{
...
#if QT_VERSION >= 0x050000 
	QDir path = QDir( QString( "%1/.quazaa/" ).arg(
						 QStandardPaths::writableLocation( QStandardPaths::HomeLocation ) ) );

	if ( !path.exists() )
		path.mkpath( QString( "%1/.quazaa/" ).arg(
						 QStandardPaths::writableLocation( QStandardPaths::HomeLocation ) ) );

	QFile f( QString( "%1/.quazaa/hostcache.dat" ).arg(
				 QStandardPaths::writableLocation( QStandardPaths::HomeLocation ) ) );
#else
	QDir path = QDir( QString( "%1/.quazaa/" ).arg(
	...
	systemLog.postLog( LogSeverity::Debug, QString( "Host Cache saved %1 hosts." ).arg(
						   m_lHosts.size() ) );
}
...
void CHostCache::load()
{
	...
	QFile file( QString( "%1/.quazaa/hostcache.dat" ).arg(
					QStandardPaths::writableLocation( QStandardPaths::HomeLocation ) ) );
#else
	QDir path = QDir( QString("%1/.quazaa/" ).arg(
						  QDesktopServices::storageLocation( QDesktopServices::HomeLocation ) ) );
	...
	if ( file.exists() && file.open( QFile::ReadOnly ) )
	{
		...
		if ( nVersion == HOST_CACHE_CODE_VERSION )
		{
			CEndPoint addr;
			QDateTime ts, lc;
			quint32 nFailures = 0;
	...
	systemLog.postLog( LogSeverity::Debug, QString( "Host Cache loaded %1 hosts." ).arg(
						   m_lHosts.size() ) );
}
