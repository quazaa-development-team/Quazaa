...
static void dump_leak(const char* pointer, const new_ptr_list_t* ptr)
{
	unsigned char *buf = (unsigned char*)pointer;
	unsigned int i, j;
	for (i = 0; i < ptr->size; i += 16)
	{
		fprintf(new_output_fp, "%08x: ", i);
		for (j = 0; j < 16; j++)
		{
			if (i + j < ptr->size )
				fprintf(new_output_fp, "%02X ", buf[i + j]);
			else
				fprintf(new_output_fp, "   ");
		}
		fprintf(new_output_fp, " ");
		...
		{
			if ( i + j < ptr->size)
				fprintf(new_output_fp, "%c", buf[i + j] >= 32 ? buf[i + j] : '.');
		}
		fprintf(new_output_fp, "\n");
	}
}
