...
#ifndef QUAZAACSSPARSER_P_H
#define QUAZAACSSPARSER_P_H
...
namespace QuazaaCss
{
	...
	struct CssValueExtractor
	{
		...
		bool extractPosition(int* l, int* t, int* r, int* b, QuazaaCss::Origin*, Qt::Alignment*,
		                     QuazaaCss::PositionMode*, Qt::Alignment*);
	...
	public:
		...
		inline bool parseNextSelector(CssSelector* sel)
		{
			if(!testSelector())
			{
				return recordError();
		...
		inline bool parseNextExpr(QVector<CssValue> *values)
		{
			if(!testExpr())
			{
				return recordError();
		...
		inline const CssSymbol& symbol() const
		{
...
Q_DECLARE_METATYPE(QuazaaCss::BackgroundData)
Q_DECLARE_METATYPE(QuazaaCss::LengthData)
Q_DECLARE_METATYPE(QuazaaCss::BorderData)
